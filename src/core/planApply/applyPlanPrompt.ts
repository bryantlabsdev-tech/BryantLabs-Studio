...
export interface BuildApplyPlanBatchPromptInput {
  ...
  /** Short project hint (framework/language); omit large file lists. */
  readonly projectHint?: string;
  /** Project Intelligence V1 block — shared planner/coder context. */
  readonly intelligenceBlock?: string;
  /** UI-only edit: notes from audit + memory instead of full intelligence. */
...
/**
 * Build the Apply Plan batch patch prompt. Only `files` appear in the prompt —
 * callers must not pass blocked or out-of-allowlist paths.
 */
...
/** Paths for UI-only direct rewrite (App + global CSS only). */
